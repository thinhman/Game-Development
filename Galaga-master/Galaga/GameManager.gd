...
# All the node grabbing this class will need
onready var player = get_node("/root/Root/Ship")
...
# Class variables
const ship_scn = preload("res://Ship.tscn")
var can_shoot = true
...
func _ready():
	...
	if SoundVariables.playGame:
		Sound.get_node("Game Music").play()
		SoundVariables.playGame = false
...
func _process(delta):
	...
	if enemyCount == 0:
		...
		if !animator.is_playing():
			player = get_node("/root/Root/Ship")
			player.showTrails()
...
func load_game():
	...
	while not save_game.eof_reached():
		...
		for i in current_line.keys():
			if i == "High_Score" or i == "Lives" or i == "Level" or i == "Previous_High_Score" or i == "Pos_x" or i == "Pos_y":
				continue
		break
	save_game.close()
